...
serialXSB::serialXSB(QObject *parent) : QObject(parent)
{
...
char serialXSB::getChecksum(char *info,int start,int bytes)
{
    int i;
    int checksum = 0;
...
void serialXSB::printBinary(int n)
{
    int i = 0;
    char abcd[10];
    strcpy(Binary,"");
...
int serialXSB::readTestData(int index,int index1,testDataStr *tds)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
    ...
    if(buffer.at(0) == 0x7B && buffer.at(1) == 0x19 && buffer.at(3) == 0x04)
    {
        ...
        switch(tds->model)
        {
            ...
            case 2:
                tds->voltage = (quint8(buffer.at(12))<<8|quint8(buffer.at(11)))/1.0;
                tds->current = (quint8(buffer.at(16))<<24|quint8(buffer.at(15))<<16|quint8(buffer.at(14))<<8|quint8(buffer.at(13)))/10.0;
                break;
            case 3:
                tds->voltage = (quint8(buffer.at(12))<<8|quint8(buffer.at(11)))/1.0;
...
void serialXSB::readGroupData(groupConfig group)
{
    ...
    for(int i=0;i<tc.itemNumMax;i++)        //测试项最大数量
    {
        delay(50);
...
int serialXSB::readMachineStatus()
{
    int i,j,k,h;
    char cmd[7];
    struct timeval tpstart,tpend;
...
void serialXSB::serialReadMessage(quint16 addr,quint16 cmd,QByteArray msg)
{
    if(cmd == 1 && addr == 0)
    {
        openSerial();
...
int serialXSB::fillCommonPhase(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialXSB::fillCommon(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialXSB::sendSysConfig(QByteArray msg)
{
    if(msg.size() != 16)
        return;
...
int serialXSB::chooseGroup(int index)
{
    if(index < 0)
        return -1;
...
void serialXSB::readGroupSetData(int g,int s)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialXSB::startTest(int index)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
