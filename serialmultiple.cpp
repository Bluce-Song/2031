...
serialMultiple::serialMultiple(QObject *parent) : QObject(parent)
{
...
void serialMultiple::openSerial()
{
    if(com  ==  NULL)
...
void serialMultiple::closeSerial()
{
    timer->stop();
...
char serialMultiple::getChecksum(char *info,int start,int bytes)
{
    int i;
    int checksum = 0;
...
void serialMultiple::delay(int d)
{
...
void serialMultiple::readStatus()
{
    if(stop == 1)
        return;
...
void serialMultiple::printBinary(int n)
{
    int i = 0;
    char abcd[10];
    strcpy(Binary,"");
...
int serialMultiple::readTestData(int index,int index1,testDataStr *tds)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialMultiple::readGroupData(groupConfig group)
{
...
int serialMultiple::readMachineStatus()
{
    int i,j,k,h;
    char cmd[7];
    struct timeval tpstart,tpend;
...
void serialMultiple::waitStateFun()
{
    int sum = 0;
    while(true)
    {
...
void serialMultiple::serialReadMessage(quint16 addr,quint16 cmd,QByteArray msg)
{
    if(cmd == 1 && addr == 0)
    {
        openSerial();
...
int serialMultiple::fillCommonPhase(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialMultiple::fillCommon(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialMultiple::sendSysConfig(QByteArray msg)
{
    if(msg.size() != 16)
        return;
...
int serialMultiple::chooseGroup(int index)
{
    if(index < 0)
        return -1;
...
void serialMultiple::readGroupSetData(int g,int s)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialMultiple::startTest(int index)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
