...
char serialPort::getChecksum(char *info,int start,int bytes)
{
    int i;
    int checksum = 0;
...
void serialPort::printBinary(int n)
{
    int i = 0;
    char abcd[10];
    strcpy(Binary,"");
...
int serialPort::readTestData(int index,int index1,testDataStr *tds)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialPort::readGroupData(groupConfig group)
{
...
int serialPort::readMachineStatus()
{
    int i,j,k,h;
    char cmd[7];
    struct timeval tpstart,tpend;
...
void serialPort::serialReadMessage(quint16 addr,quint16 cmd,QByteArray msg)
{
    if(cmd == 1 && addr == 0)
    {
        openSerial();
...
int serialPort::fillCommonPhase(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialPort::fillCommon(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialPort::sendSysConfig(QByteArray msg)
{
    if(msg.size() != 16)
        return;
...
int serialPort::chooseGroup(int index)
{
    if(index < 0)
        return -1;
...
void serialPort::readGroupSetData(int g,int s)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialPort::startTest(int index)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
