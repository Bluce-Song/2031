...
serialportch::serialportch(QObject *parent) : QObject(parent)
{
...
char serialportch::getChecksum(char *info,int start,int bytes)
{
    int i;
    int checksum = 0;
...
void serialportch::printBinary(int n)
{
    int i = 0;
    char abcd[10];
    strcpy(Binary,"");
...
int serialportch::readTestData(int index,int index1,testDataStr *tds)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialportch::readGroupData(groupConfig group)
{
    ...
    for(int i=0;i<tc.itemNumMax;i++)
    {
        ...
        if(i < group.itemNum)
        {
            ...
            {
                if(group.items.at(i).state == 1)
                    ch = "01";
                else
                    ch = "00";
            }

            QString tt = ch+"10"+(QString)Binary;
            char *e;
            dd = tt.toLatin1();
            id = strtol(dd,&e,2);
...
int serialportch::readMachineStatus()
{
    int i,j,k,h;
    char cmd[7];
    struct timeval tpstart,tpend;
...
void serialportch::waitStateFun()
{
    int sum = 0;
    while(true)
    {
...
void serialportch::serialReadMessage(quint16 addr,quint16 cmd,QByteArray msg)
{
    if(cmd == 1 && addr == 0)
    {
        openSerial();
...
int serialportch::fillCommonCH(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
    ...
    cmd[24] = ch&0xff;
    cmd[25] = ch>>8;
    cmd[26] = getChecksum(cmd,1,25);
    cmd[27] = 0x7D;

    int i = com->write(cmd,28);
...
int serialportch::fillCommon(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialportch::sendSysConfig(QByteArray msg)
{
    if(msg.size() != 16)
        return;
...
int serialportch::chooseGroup(int index)
{
    if(index < 0)
        return -1;
...
void serialportch::readGroupSetData(int g,int s)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialportch::startTest(int index)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
