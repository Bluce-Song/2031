    ...
    ui(new Ui::selfcheck)
{
    ...
    connect(ui->lineEdit,SIGNAL(returnPressed()),this,SLOT(on_loginOK_clicked()));
...
void selfcheck::on_loginOK_clicked()
{
...
void selfcheck::sendGroupToMainThread()
{
    if(rowNum == 0)
        return;
 ...
 * 说明：
*************************************/
void selfcheck::openGroup(QModelIndex index)
{
    if(userLevel == 0)
    {
        QMessageBox box(QMessageBox::Warning,tr("测试程序"),tr("您无权限进行此操作！"));
 ...
 * 说明：
*************************************/
void selfcheck::readSelfCheckGroup()
{
    //连接数据库
 ...
 * 说明：
*************************************/
void selfcheck::readGroup(QString name)
{
    //连接数据库
    QString sDbNm = QCoreApplication::applicationDirPath()+"/db/SysSet.mdb";      //数据库文件
    QSqlDatabase db;
