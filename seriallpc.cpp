...
serialLPC::serialLPC(QObject *parent) : QObject(parent)
{
...
char serialLPC::getChecksum(char *info,int start,int bytes)
{
    int i;
    int checksum = 0;
...
void serialLPC::printBinary(int n)
{
    int i = 0;
    char abcd[10];
    strcpy(Binary,"");
...
int serialLPC::readTestData(int index,int index1,testDataStr *tds)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialLPC::readGroupData(groupConfig group)
{
    ...
    for(int i=0;i<tc.itemNumMax;i++)        //当前限制每个测试程序只有八个测试项
    {
        delay(50);
        ...
        if(i < group.itemNum)
        {
            ...
            case 6:
            {
                ...
                a = QString("%1").arg(group.items.at(i).data[0]*1.0);
                v = a.toInt();
                a = QString("%1").arg(group.items.at(i).data[6]*1.0);
                t1 = a.toInt();
            }
                break;
            case 7:
...
int serialLPC::readMachineStatus()
{
    int i,j,k,h;
    char cmd[7];
    struct timeval tpstart,tpend;
...
void serialLPC::serialReadMessage(quint16 addr,quint16 cmd,QByteArray msg)
{
    if(cmd == 1 && addr == 0)
    {
        openSerial();
...
int serialLPC::fillCommonPhase(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialLPC::fillCommon(groupConfig group)
{
    char cmd[32];
    struct timeval tpstart,tpend;
    float timeuse;
...
void serialLPC::sendSysConfig(QByteArray msg)
{
    if(msg.size() != 16)
        return;
...
int serialLPC::chooseGroup(int index)
{
    if(index < 0)
        return -1;
...
void serialLPC::readGroupSetData(int g,int s)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
...
int serialLPC::startTest(int index)
{
    char cmd[7];
    struct timeval tpstart,tpend;
    float timeuse;
